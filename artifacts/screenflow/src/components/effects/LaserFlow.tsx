...
export function LaserFlow(props: LaserFlowProps) {
  ...
  useEffect(() => {
    ...
    const settings = { ...DEFAULT_PROPS, ...propsRef.current };
    ...
    const { r, g, b } = hexToRGB(settings.color ?? "#FFFFFF");

    const uniforms: Uniforms = {
      iTime: { value: 0 },
      iResolution: { value: new THREE.Vector3(1, 1, 1) },
      iMouse: { value: new THREE.Vector4(0, 0, 0, 0) },
      uWispDensity: { value: settings.wispDensity },
      uTiltScale: { value: settings.mouseTiltStrength },
      uFlowTime: { value: 0 },
      uFogTime: { value: 0 },
      uBeamXFrac: { value: settings.horizontalBeamOffset },
      uBeamYFrac: { value: settings.verticalBeamOffset },
      uFlowSpeed: { value: settings.flowSpeed },
      uVLenFactor: { value: settings.verticalSizing },
      uHLenFactor: { value: settings.horizontalSizing },
      uFogIntensity: { value: settings.fogIntensity },
      uFogScale: { value: settings.fogScale },
      uWSpeed: { value: settings.wispSpeed },
      uWIntensity: { value: settings.wispIntensity },
      uFlowStrength: { value: settings.flowStrength },
      uDecay: { value: settings.decay },
      uFalloffStart: { value: settings.falloffStart },
    ...
    const setSizeNow = () => {
      ...
      (uniforms.iResolution.value as THREE.Vector3).set(w * currentDpr, h * currentDpr, currentDpr);
    ...
    const updateMouse = (clientX: number, clientY: number) => {
      ...
      mouseTarget.set(x * ratio, canvasRect.height * ratio - y * ratio);
    };
    ...
    const adjustDprIfNeeded = (now: number) => {
      ...
      if (avgFps < lowerThresh) {
        next = clampVal(currentDpr * 0.9, dprFloor, baseDpr);
      } else if (avgFps > upperThresh && currentDpr < baseDpr) {
        next = clampVal(currentDpr * 1.05, dprFloor, baseDpr);
      }
      if (Math.abs(next - currentDpr) > 0.01) {
        currentDpr = next;
        setSizeNow();
      }
      fpsSamples.length = 0;
    ...
    const animate = () => {
      ...
      const tau = Math.max(1e-3, propsRef.current.mouseSmoothTime ?? DEFAULT_PROPS.mouseSmoothTime);
  ...
  useEffect(() => {
    ...
    uniforms.uWispDensity.value = props.wispDensity ?? DEFAULT_PROPS.wispDensity;
    uniforms.uTiltScale.value = props.mouseTiltStrength ?? DEFAULT_PROPS.mouseTiltStrength;
    uniforms.uBeamXFrac.value = props.horizontalBeamOffset ?? DEFAULT_PROPS.horizontalBeamOffset;
    uniforms.uBeamYFrac.value = props.verticalBeamOffset ?? DEFAULT_PROPS.verticalBeamOffset;
    uniforms.uFlowSpeed.value = props.flowSpeed ?? DEFAULT_PROPS.flowSpeed;
    uniforms.uVLenFactor.value = props.verticalSizing ?? DEFAULT_PROPS.verticalSizing;
    uniforms.uHLenFactor.value = props.horizontalSizing ?? DEFAULT_PROPS.horizontalSizing;
    uniforms.uFogIntensity.value = props.fogIntensity ?? DEFAULT_PROPS.fogIntensity;
    uniforms.uFogScale.value = props.fogScale ?? DEFAULT_PROPS.fogScale;
    uniforms.uWSpeed.value = props.wispSpeed ?? DEFAULT_PROPS.wispSpeed;
    uniforms.uWIntensity.value = props.wispIntensity ?? DEFAULT_PROPS.wispIntensity;
    uniforms.uFlowStrength.value = props.flowStrength ?? DEFAULT_PROPS.flowStrength;
